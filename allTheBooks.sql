...
select titulo, idpedido, idcliente, fechapedido, fechaexped
from pedido natural join libros_pedido natural join libro;
...
select idpedido, count(titulo) "titulos", sum(cantidad*precioventa) "importe"
from libros_pedido natural join libro natural join pedido
group by idpedido
having count(titulo) > 1;
--18. Pedidos con importe total que contengan más de 4 libros (ejemplares)
select idpedido, sum(cantidad) "CANTIDAD" , sum(precioventa*cantidad) "importe"
from libros_pedido natural join libro
group by idpedido
having sum(Cantidad) > 4;

--19. Lista de libros más caros (Dime el libro mas caro!!!).
...
from libro 
where isbn not in
(select distinct isbn from libros_pedido)
...
FROM (((cliente Cl left join pedido P on Cl.idcliente = P.idcliente) left join libros_pedido LP on LP.idpedido=P.idpedido) left join libro L on LP.isbn=L.isbn);

select nombre, idpedido, isbn, titulo
from (( Cliente cl left join Pedido ped on cl.idcliente = ped.idcliente) left join libros_pedido lp on ped.idpedido = lp.idpedido) left join libro lib on lp.isbn = lib.isbn;   